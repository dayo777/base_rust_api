...
COPY src ./src/
RUN . scripts/target.sh && touch src/main.rs && cargo build --release --target $RUST_TARGET && cp target/$RUST_TARGET/release/base_rust_api target/base_rust_api
...
COPY --from=builder /app/target/base_rust_api .
